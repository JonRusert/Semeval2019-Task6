...
#Example run: python3 evaluate.py Kim-CNN_random_out offenseval-training-v1.tsv
...
def evaluate(outputFile, goldFile):
    ...
    with open(goldFile, 'r') as csvfile:
        ...
        for curGold in goldreader:
            gold[curGold[0].strip()] = curGold[2].strip()
            if(curGold[2].strip() not in classCor):
                classCor[curGold[2].strip()] = 0.0
                classAtt[curGold[2].strip()] = 0.0
                classTot[curGold[2].strip()] = 0.0
